...
### 修正
![](assets/2025-01-24-22-51-29.png)
### そら似
![](assets/2025-01-24-22-57-06.png)
## 補足②: データセット作成方針
顔データセットには多様性が重要ですが、どこまでの多様性を含めるか前もって方針をたてておかねばなりません。

### 年齢
![](assets/2025-01-24-23-13-10.png)
### 表情

### 使用するレンズ特性
![](assets/2025-01-24-23-10-57.png)

### プリント・画面表示
![](assets/2025-01-24-22-59-47.png)

## 補足③: 顔データセットのこれから
### CNNからViTへ
顔認証とは字のごとく「顔」を対象としているので、総合的に考えてCNNが最適です。しかし人間は顔のみを個人の識別に使っていません。その人の顔以外に、体格や服装も識別の多くの部分を占めます。
最終的には顔で識別をしますが、たとえば友人に後ろから声をかける時、あなたは顔を見ずに個人を特定しています。
このような場合、技術的にはたとえば過去の記事ではDEEPSORTやDAMO-YOLOなどを扱いましたが、ViTこそ真価を発揮するかもしれません。
そうした場合、顔を含めた全身の画像が必要となり、またTransformerゆえに膨大な大きさのデータセットになることが予想されます。
### 人種バイアスから細分化へ
顔データセットの論文では人種の比率が必ず書いてありますが、その分類はかなり大雑把なものです。顔データセットの人種の比率が認識のバイアスとなることはかなり有名ですが、そもそもの人種の分け方にも問題があります。
日本人は東アジア人に含まれますが、東アジアといっても日本人、漢民族、満州族、回族、ウイグル人、チベット人、チワン族、チャン族、ミャオ族、トゥチャ族、韓民族、カザフ人、ザイ族が存在します。
日本人と中国人の顔つきは違いますし、それがバイアスとなってあらわれます。
しかし日本人しかいない環境で使うことを想定した場合、わざわざすべての民族を1:1で用意する必要はないはずです。
この場合は日本人だけの顔データセットを作成すればよく、そのほうがかえってバイアスがかからずに正しく認証できます。
これがアメリカで使用する想定ならそういうわけにはいきません。
特定環境で使用することを想定して、データセットは細分化されていくと予想されます。
